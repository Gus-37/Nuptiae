...
export default function HotelesScreen({ navigation }) {
  const { colors, fontScale, theme } = useUISettings();
  const [proveedores, setProveedores] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const path = 'products/hoteles';
    const proveedoresRef = ref(database, path);
    const unsub = onValue(proveedoresRef, (snapshot) => {
      const data = snapshot.val();
      console.log('[HotelesScreen] Firebase path:', path);
      console.log('[HotelesScreen] Data:', data);
      if (data) {
      ...
      console.error('[HotelesScreen] Error leyendo datos:', error);
      setProveedores([]);
      setLoading(false);
    });
    return () => unsub();
    ...
    <SafeAreaView style={[styles.safeArea, { backgroundColor: colors.bg }]} edges={['top', 'left', 'right']}>
      ...
      <View style={[styles.container, { backgroundColor: colors.bg }]}>
        <View style={[styles.header, { backgroundColor: colors.card, borderBottomColor: colors.border }]}>
          ...
          <Text style={[styles.headerTitle, { color: colors.text, fontSize: 18 * fontScale }]}>Hoteles</Text>
          <View style={styles.headerButton} />
        </View>
        {loading ? (
          <View style={styles.loadingContainer}>
            <Text style={[styles.helper, { color: colors.muted, fontSize: 14 * fontScale }]}>Cargando hoteles...</Text>
          </View>
        ) : proveedores.length === 0 ? (
          <View style={styles.emptyContainer}>
            <Text style={[styles.helper, { color: colors.muted, fontSize: 14 * fontScale }]}>No hay hoteles registrados. Verifica Firebase en products/hoteles.</Text>
          </View>
        ) : (
          <FlatList
            data={proveedores}
